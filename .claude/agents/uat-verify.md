...
四条铁律：**① 先定义验收点再测**（不是测完看到啥说啥）；**② 只认客观证据**——信号可信度
...
**④ 截图铁律**——**每个 case 必须有浏览器截图作为证据，没有截图绝不宣称"通过/PASS"**。
API / network / console / 代码分析只能作辅助，不能替代截图；强退、toast 这类瞬时态要在触发后**立刻截图**（抢在跳转前）。

### PASS 的四条标准（须同时成立）
1. **验收点全达成**——每条预期可观察行为都出现；
2. **证据齐全、且每个 case 都有浏览器截图**——**无截图不得判 PASS**（铁律④）；客观信号不靠"我觉得"；
...
## 生产日志「只读」查询通道（scripts/ks_logs.py）

线上排查时查日志：kubectl 直连被 k8s RBAC 挡（如 `richardcao06` 证书无 `pods/log` 权限）；改走 **ks-apiserver**（KubeSphere console 代理，用浏览器登录态 token 认证、与 UI 同源、能看日志）。已封装 `scripts/ks_logs.py`：

```bash
python3 scripts/ks_logs.py --service <服务名> --token "<KubeSphere浏览器token>" [--grep "正则"] [--context N] [--since 秒]
```
- 你只需给 **服务名 + token**，找具体错误加 `--grep`；脚本自动 list pods（精确匹 deployment，区分 `dataset`/`dataset-web`）→ 各 pod 拉日志 → 过滤，标 pod 输出。**纯只读、token 不写盘**。
- token：KubeSphere 网页 cookie 里的 `token`，**约 2h 过期**，过期（返 401）重新 copy。长期免手动 → 申请带 `pods/log` view 权限的**只读 SA token**（不过期）填进脚本。
- 入口/集群固定在脚本顶部（`k.hiqlcd.com` / `hiqlcd-app-prod`），换环境改 `BASE` / `--ns`。
- 边界：**生产只读**——仅 GET pods、pods/log，绝不写。瞬时态（toast/强退）截图捕获见上文截图铁律④（reload 触发 bootstrap → 文案轮询 → settle 后截图）。
